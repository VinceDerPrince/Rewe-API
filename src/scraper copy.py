...
def _generate_markets(searchTerm: str) -> json:
    ...
    url = f"https://www.rewe.de/api/marketsearch?searchTerm={searchTerm}"
...
def _all_offers(searchTerm: str) -> Dict:
    ...
    for market in markets:
        ...
        marketName = market["marketHeadline"] + " " + market["contactStreet"] + " " + market["contactZipCode"]
        url = f"https://www.rewe.de/api/all-stationary-offers/{marketId}"
        urls.append(url)
        ids.append(marketId)
        names.append(marketName)
